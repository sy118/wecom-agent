...
### 需求:企微运行时激活上下文路由
系统必须在静态 Binding 和默认上下文之外支持企微运行时激活上下文，并在处理普通消息时优先使用该激活上下文。

#### 场景:用户在会话内激活上下文
- **当** 用户在 `chatKey=wecom:group:g1` 中成功执行 `/ctx use ctx-a`
- **那么** 该用户后续在同一 chatKey 内发送的普通消息必须使用 `ctx-a`

#### 场景:激活上下文优先于静态绑定
- **当** `chatKey=wecom:group:g1` 静态绑定到 `ctx-default`
- **并且** 用户在该 chatKey 内激活了 `ctx-a`
- **那么** 系统必须使用 `ctx-a` 处理该用户的普通消息

#### 场景:无激活上下文时回退静态绑定
- **当** 用户未设置运行时激活上下文
- **那么** 系统必须继续使用现有 Binding 路由规则

#### 场景:无静态绑定时回退默认上下文
- **当** 用户未设置运行时激活上下文且 chatKey 无静态 Binding
- **那么** 系统必须继续使用 Bot 默认上下文

### 需求:查看当前上下文
系统必须支持用户在企微中查看当前实际生效的上下文及其来源。

#### 场景:当前上下文来自激活上下文
- **当** 用户发送 `/ctx current`
- **并且** 当前使用运行时激活上下文
- **那么** 系统必须返回上下文名称和来源为运行时切换

#### 场景:当前上下文来自静态绑定
- **当** 用户发送 `/ctx current`
- **并且** 当前使用 Binding
- **那么** 系统必须返回上下文名称和来源为后台绑定

### 需求:重置激活上下文
系统必须支持用户清除自己的运行时激活上下文，并回退到 Binding 或默认上下文。
