...
import {
  Alert, Button, Card, Divider, Form, Input, InputNumber, Modal, Popconfirm, Select,
  Space, Switch, Table, Tag, message,
} from 'antd'
import { ArrowLeftOutlined, PlusOutlined } from '@ant-design/icons'
import { useNavigate, useParams } from 'react-router-dom'
import { botsApi, contextsApi, mcpServersApi, skillsApi } from '../api/index.js'
...
export default function ContextsPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [ctxList, mcpList, skillList, botData, defaults] = await Promise.all([
    ...
    { title: '名称', dataIndex: 'name', key: 'name' },
    {
      ...
      render: (cfgs: McpConfig[] = []) => {
        ...
        return enabled.map((cfg) => {
          ...
          return (
            <Tag key={cfg.mcpServerId} color="blue">
    ...
    <div>
      ...
      >
        <Form form={form} onFinish={handleSave} layout="vertical">
          ...
          {bot?.provider === 'dify' ? (
            <Alert type="info" message="该机器人使用 Dify 工作流，工具调用由 Dify 内部处理" showIcon style={{ marginBottom: 16 }} />
          ) : mcpServers.length === 0 ? (
