...
修改 MCP 服务 URL、启用状态、上下文 MCP 配置、群聊绑定或 Skill 配置后，运行中的机器人会自动刷新运行时配置。修改企业微信凭证、LLM 模型、Provider、Dify 或视觉识别等机器人基础配置时，系统会自动重启正在运行的对应机器人，使新配置生效。
...
- 机器人提示没有 Wiki 工具：确认 MCP 服务器已启用，URL 是 `/sse` 结尾，上下文中已启用该 MCP 配置；保存后运行中的机器人会自动刷新工具池。
