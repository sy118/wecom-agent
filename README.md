...
- 上下文与绑定：为不同群聊或用户配置独立系统提示、会话 TTL、MCP 工具和 Skill 能力；每个机器人可开启“免绑定会话访问”，让未绑定私聊和群聊使用默认上下文。
- MCP 与 Skill：支持 `sse`、`stdio`、`streamable-http` MCP Server，以及可上传的 Skill bundle。
- MCP 独立检测：管理页可在机器人停止时检测 MCP 配置，返回校验、连接、初始化、工具加载和关闭阶段的耗时与脱敏错误；检测结果即时返回，不写入运行时工具池或历史记录。
...
6. 在绑定管理中按机器人账号选择是否允许未绑定会话访问。开启后使用该机器人标记为 `isDefault` 的上下文；静态绑定和运行时上下文优先。未配置默认上下文时仍会拒绝访问。关闭后未绑定会话会记录为 `needs-binding` 并返回绑定提示。
7. 将企业微信群聊或单聊绑定到上下文。
8. 启动机器人，并在会话监控中观察消息处理情况。
9. 如需自动化，创建定时任务并填写 cron 表达式和目标会话。

绑定策略接口：`GET/PUT /api/bots/:botId/bindings/settings`，更新会热刷新运行中的机器人。MCP 检测接口：`POST /api/mcp-servers/:id/test`。未绑定会话发现记录只保存在运行时内存，机器人重启后不会保留历史。
...
| `MCP_TOOL_TIMEOUT_MS` | MCP 工具调用超时，可选 | `180000` |
